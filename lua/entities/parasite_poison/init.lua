...
function ENT:Think()
	if self.depleted or !self.poisonent or !ValidEntity( self.poisonent ) or !self.poisonent.poisoned or self.poisonent:Health() <= 0 then self:Remove() end
...
function ENT:OnRemove()
	if self.poisonent and ValidEntity( self.poisonent ) then self.poisonent.poisoned = false end
end
