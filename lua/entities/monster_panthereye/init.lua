...
ENT.Model = "models/panthereye.mdl"
ENT.RangeDistance		= 400
ENT.MeleeDistance		= 100
ENT.allow_range_attacking = true

ENT.SpawnRagdollOnDeath = true
ENT.FadeOnDeath = false
ENT.BloodType = "green"
ENT.Pain = true
ENT.PainSound = "npc/panthereye/pe_pain"
ENT.PainSoundCount = 2
ENT.DeathSound = "npc/panthereye/pe_die"
...
local schdPrepAttack = ai_schedule.New( "Prepare Attack" ) 
schdPrepAttack:EngTask( "TASK_STOP_MOVING", 0 )
schdPrepAttack:EngTask( "TASK_STOP_MOVING", 0 )
schdPrepAttack:EngTask( "TASK_PLAY_SEQUENCE_FACE_ENEMY", ACT_FLINCH_RIGHTLEG )
//schdPrepAttack:EngTask( "TASK_PLAY_SEQUENCE_FACE_ENEMY", "subtle_motion" )
schdPrepAttack:AddTask( "attack_jump" )
schdPrepAttack:EngTask( "TASK_PLAY_SEQUENCE_FACE_ENEMY", ACT_RANGE_ATTACK1 )
...
function ENT:Initialize()
	...
	if !self.health then
		self:SetHealth(sk_panthereye_health_value)
	end
	
	if self.triggertarget and self.triggercondition == "3" then self.starthealth = self:Health() end
	
	self.attacksound1 = CreateSound( self, "npc/panthereye/pe_attack1.wav" )
	self.attacksound2 = CreateSound( self, "npc/panthereye/pe_attack2.wav" )
	self.attacksound3 = CreateSound( self, "npc/panthereye/ag_attack2.wav" )
	self.attacksound4 = CreateSound( self, "npc/panthereye/ag_attack3.wav" )
	
	self.idlesound1 = CreateSound( self, "npc/panthereye/pe_idle1.wav" )
	self.idlesound2 = CreateSound( self, "npc/panthereye/pe_idle2.wav" )
	self.idlesound3 = CreateSound( self, "npc/panthereye/pe_idle3.wav" )
	
	self.alertsound = "npc/panthereye/pe_alert"
	self.alertsound_amount = 2
	...
	self.possess_viewpos = Vector( -100, 0, 90 )
...
function ENT:Think()
	...
	if self.possessed and !self.attacking and ( !self.possession_allowdelay or ( self.possession_allowdelay and CurTime() > self.possession_allowdelay ) ) then
		...
		elseif self.master:KeyDown( 2048 ) then
			self.attacking = true
			self.allow_range_attacking = false
			self.idle = 0
			self:StartSchedule( schdPrepAttack )
		end
	end
...
function ENT:TaskStart_attack_jump()
	timer.Destroy( "attack_end_timer" .. self.Entity:EntIndex( ) )
	timer.Create( "Attack_timer" .. self:EntIndex(), 0.4, 1, function() self:Attack_Range() end )
	self:TaskComplete()
end 

function ENT:Task_attack_jump()
end
...
function ENT:Attack_Range()
	...
	self.attacksound4:Stop()
	
	local attacksound_rand = math.random(1,4)
	...
	if (attacksound_rand == 4) then
		self.attacksound4:Play()
	end	
	local function Attack_jump()
		if (self.enemy and ValidEntity(self.enemy)) or self.possessed then
			...
			if !self.possessed then
				...
				enemy_pos.x = 0
				enemy_pos.y = 0
				enemy_dist_z = entity_pos:Distance( enemy_pos )
			end
			local enemy_pos
			if !self.possessed then
				enemy_pos = self.enemy:GetPos()
			else
				enemy_pos = self:GetPos() -Vector( 0, 0, 45 ) +self:GetForward() *25
			end
			if !self.possessed and enemy_dist_z > 15 then 
				enemy_pos.z = self:GetPos().z +15
			end
			local attack_veloc = enemy_pos + Vector( 0, 0, 50 ) - self:GetPos()
			self:SetVelocity( attack_veloc:Normalize() *1300 )
		end
		local enemy_table = {}
		local function attack_dmg()
			local self_pos = self:GetPos()
			
			local victim = ents.FindInSphere( self_pos, 65 )
			for k, v in pairs(victim) do
				if( !table.HasValue( enemy_table, v ) ) then
					if( ( ( ( v:IsPlayer() and v:Alive() ) or v:IsNPC() ) and ( self:Disposition( v ) == 1 or self:Disposition( v ) == 2 ) ) or v:GetClass() == "prop_physics" ) then
						...
						if v:IsNPC() and v:Health() - sk_panthereye_jump_value <= 0 then
							self.killicon_ent = ents.Create( "sent_killicon" )
							self.killicon_ent:SetKeyValue( "classname", "sent_killicon_panthereye" )
						...
						v:TakeDamage( sk_panthereye_jump_value, self, self.attack_inflictor )  
						v:EmitSound( "npc/zombie/claw_strike" ..math.random(1,3).. ".wav", 100, 100)
						//v:SetVelocity( v:LocalToWorld( Vector( -300, 0, 300 ) ) )
			...
			local function attack_end()
				...
				timer.Create( "Allow_range_timer" .. self:EntIndex(), math.random( 3, 6 ), 1, function() self.allow_range_attacking = true end )
			end
			if( !timer.IsTimer( "attack_end_timer" .. self.Entity:EntIndex( ) ) ) then
	...
	timer.Create( "Attack_jump_timer" .. self:EntIndex(), 0.4, 1, Attack_jump )
end
...
function ENT:Attack_Melee()
	...
	local function attack_dmg()
		local self_pos = self:GetPos()
		local victim = ents.FindInBox( self:LocalToWorld( Vector( 28, 33, 4 ) ), self:LocalToWorld( Vector( 97, -30, 35 ) ) )
		...
		for k, v in pairs(victim) do
			if( ( ( ( v:IsPlayer() and v:Alive() ) or v:IsNPC() ) and ( self:Disposition( v ) == 1 or self:Disposition( v ) == 2 ) ) or v:GetClass() == "prop_physics" ) then
				...
				if v:IsNPC() and v:Health() - sk_panthereye_slash_value <= 0 then
					self.killicon_ent = ents.Create( "sent_killicon" )
					self.killicon_ent:SetKeyValue( "classname", "sent_killicon_panthereye" )
					self.killicon_ent:Spawn()
					self.killicon_ent:Activate()
				...
				v:TakeDamage( sk_panthereye_slash_value, self, self.attack_inflictor )  
				if v:IsPlayer() then
	...
	timer.Create( "melee_attack_dmgdelay_timer" .. self.Entity:EntIndex( ), 0.5, 1, attack_dmg )
end
...
function ENT:SelectSchedule()
	...
	if( ( self.FoundEnemy or self.FoundEnemy_fear ) and !self.attacking and !self.possessed and convar_ai == 0 ) then
		...
		if( self.enemy and ValidEntity( self.enemy ) and self.enemy:GetPos():Distance( self:GetPos() ) <= self.closest_range ) then
			...
			enemy_pos.x = 0
			enemy_pos.y = 0
			local enemy_dist_z = entity_pos:Distance( enemy_pos )
			if( self.enemy:GetPos():Distance( Pos ) < self.RangeDistance and self.enemy:GetPos():Distance( Pos ) > 100 and self.allow_range_attacking == true and self.enemy:GetPos():Distance( Pos ) > self.MeleeDistance and enemy_dist_z < 15 and self:HasCondition( 10 ) and !self:HasCondition( 42 ) ) then
...
function ENT:OnRemove()
	...
	timer.Destroy( "Attack_timer" .. self:EntIndex() )
	timer.Destroy( "entity_index_remove_timer" .. self.Entity:EntIndex( ) )
	timer.Destroy( "Allow_range_timer" .. self:EntIndex() )
	timer.Destroy( "attack_end_timer" .. self.Entity:EntIndex( ) )
	timer.Destroy( "attack_dmgdelay_timer" .. self.Entity:EntIndex( ) )
	timer.Destroy( "Attack_jump_timer" .. self:EntIndex() )
	timer.Destroy( "entity_index_remove_timer" .. self.Entity:EntIndex( ) )
