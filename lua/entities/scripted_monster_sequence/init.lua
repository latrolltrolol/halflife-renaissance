...
function ENT:Think()
	if self.targetnpc and ValidEntity( self.targetnpc ) and self.retry and !self.targetnpc.InSequence then
	...
	if self.sradius and self.start then
		...
		for k, v in pairs( ents.FindInSphere( self:GetPos(), self.sradius ) ) do
			if ValidEntity( v ) and ( v:GetName() == self.targetnpc_stringname or v:GetClass() == self.targetnpc_stringname ) and v:Health() > 0 then
	...
	elseif self.startonspawn and self.start then
		...
		if ValidEntity( target ) and target:Health() > 0 then
	...
	if self.NPCMovedToPosition and !self.onlymoveto then
		if !ValidEntity( self.targetnpc ) then self.NPCMovedToPosition = false; return end
...
function ENT:AcceptInput( cvar_name, activator, caller )
	...
	if( string.find( cvar_name,"BeginSequence" ) and ( ( caller:IsPlayer() and caller:IsAdmin() ) or !caller:IsPlayer() ) ) then
		...
		//for k, v in pairs( self:GetTargets( self.targetnpc_stringname ) ) do
			...
			if ValidEntity( v ) and v:Health() > 0 and !v.InSequence then
				self:Action( false, v )
			elseif ValidEntity( v ) and v:Health() > 0 and v.InSequence then
				self.targetnpc = v
