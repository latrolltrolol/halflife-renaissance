...
ENT.BloodType = "green"
ENT.Pain = false
ENT.DeathSound = "npc/parasite/fz_scream"
...
local schdMoveToWall = ai_schedule.New( "Move to wall" )
...
local schdClimbMount = ai_schedule.New( "Climb mount" ) 
...
local schdClimbDismount = ai_schedule.New( "Climb Dismount" ) 
...
local schdClimb = ai_schedule.New( "Climb" ) 
...
function ENT:Initialize()
	...
	self.clawstrikesound1 = CreateSound( self, "npc/parasite/claw_strike1.wav" )
	self.clawstrikesound2 = CreateSound( self, "npc/parasite/claw_strike2.wav" )
	self.clawstrikesound3 = CreateSound( self, "npc/parasite/claw_strike3.wav" )
	
	self.clawstrikesound_miss1 = CreateSound( self, "npc/parasite/claw_miss1.wav" )
	self.clawstrikesound_miss2 = CreateSound( self, "npc/parasite/claw_miss2.wav" )
	
	self.idlesound1 = CreateSound( self, "npc/parasite/idle1.wav" )
	self.idlesound2 = CreateSound( self, "npc/parasite/idle2.wav" )
	self.idlesound3 = CreateSound( self, "npc/parasite/idle3.wav" )
	
	self.alertsound = "npc/parasite/pa_alert"
...
function ENT:Think()
	...
	if self.possessed and !self.melee_attacking and !self.moving and ( !self.possession_allowdelay or ( self.possession_allowdelay and CurTime() > self.possession_allowdelay ) ) then
...
function ENT:Attack_Melee( poss, poison )
	...
	if poison then
		...
		self.clawstrikesound1:ChangePitch( 50 )
		self.clawstrikesound2:ChangePitch( 50 )
		self.clawstrikesound3:ChangePitch( 50 )
	else
		self.clawstrikesound1:ChangePitch( 100 )
		self.clawstrikesound2:ChangePitch( 100 )
		self.clawstrikesound3:ChangePitch( 100 )
		self:StartSchedule( schdMeleeAttack )
	end
	
	self.clawstrikesound1:Stop()
	self.clawstrikesound2:Stop()
	self.clawstrikesound3:Stop()
	
	self.clawstrikesound_miss1:Stop()
	self.clawstrikesound_miss2:Stop()
	...
	local function attack_dmg()
		...
		if self.enmyinrange then
			...
			if (attacksound_rand == 1) then
				self.clawstrikesound1:Play()
			elseif (attacksound_rand == 2) then
				self.clawstrikesound2:Play()
			elseif (attacksound_rand == 3) then
				self.clawstrikesound3:Play()
			end
		else
			local attackmisssound_rand = math.random(1,2)
			if (attackmisssound_rand == 1) then
				self.clawstrikesound_miss1:Play()
			elseif (attackmisssound_rand == 2) then
				self.clawstrikesound_miss2:Play()
			end
		end
...
function ENT:SelectSchedule()
	...
	if( self.following and !self.possessed ) then
		if ValidEntity( self.follow_target ) then
			...
			if( self:GetPos():Distance( self.follow_target ) > 175 and ( ( ValidEntity( self.enemy ) and self.enemy != self.follow_target and self.enemy:GetPos():Distance( self.follow_target:GetPos() ) > 800 ) or !ValidEntity( self.enemy ) ) and !self.melee_attacking and convar_ai == 0 ) then
...
function ENT:OnRemove()
	...
	if self.movetarget and ValidEntity( self.movetarget ) then self.movetarget:Remove() end
	self:EndPossession()
