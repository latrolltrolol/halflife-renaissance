...
function ENT:KeyValue( key, value )
	...
	if value == "0" then //ValidEntity( ents.FindByName( key )[1] ) // Value == 1??
	...
	if( string.find( cvar_name,"Trigger" ) and ( ( caller:IsPlayer() and caller:IsAdmin() ) or !caller:IsPlayer() ) ) then
		for k, v in pairs( self.output ) do
			...
			//	if output == "Open" then
					...
					timer.Create( tostring(self) .. "_outputdelaytimer" .. self.Fired, tonumber(output_delay), 1, function() if ValidEntity(v) then v:Fire( output, output_params, 0 ) end end )
