...
ENT.Model = "models/baby_headcrab.mdl"
ENT.MinDistance		= 235
...
function ENT:Initialize()
	...
	if !self.health then
		self:SetHealth(sk_babycrab_health_value)
	end
	...
	self.enemyTable_LI = { "monster_headcrab", "monster_zombie", "npc_fastzombie_torso", "npc_fastzombie", "npc_poisonzombie", "npc_zombie", "npc_zombie_torso", "npc_zombine", "npc_headcrab_black", "npc_headcrab_poison", "npc_headcrab_fast", "npc_headcrab", "monster_bigmomma" }
	
	self:SetSchedule( 1 )
	
	self.enemyTable_enemies_e = {}
	
	if gonarch_owner and ValidEntity( gonarch_owner ) then
		self.owner = gonarch_owner
		...
		for k, v in pairs( npcs ) do
			if ValidEntity( v ) then
				...
				self:AddEntityRelationship( v, self.owner:Disposition( v ), 10 )
			end
		end
	end
...
function ENT:Attack( poss )
	...
	self:EmitSound( "npc/headcrab/attack" .. math.random(1,3) .. ".wav", 100, 140)
	local enemy_pos
	if !poss then
		...
		for k, v in pairs(victim) do
			if( !table.HasValue( enemy_table, v ) ) then
				if( ( ( ( v:IsPlayer() and v:Alive() ) or v:IsNPC() ) and ( self:Disposition( v ) == 1 or self:Disposition( v ) == 2 ) ) or v:GetClass() == "prop_physics" ) then
					self:EmitSound( "npc/headcrab/headbite.wav", 100, 120)
					table.insert( enemy_table, v )
					if v:IsNPC() and v:Health() - sk_babycrab_melee_value <= 0 then
						self.killicon_ent = ents.Create( "sent_killicon" )
						self.killicon_ent:SetKeyValue( "classname", "sent_killicon_babycrab" )
					...
					v:TakeDamage( sk_babycrab_melee_value, self, self.attack_inflictor )  
